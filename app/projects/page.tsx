...
export default function Projects() {
    ...
    return (
        <div className="min-h-screen py-40 flex flex-col items-center justify-center bg-[#EAE0CF]">


            <div className="relative w-full max-w-6xl flex items-center justify-center px-4 md:px-0">
                {/* Left Arrow */}
                <AnimatePresence>
                    {inspectedIndex === null && (
                        <motion.button
                            ...
                            exit={{ opacity: 0, x: -20 }}
                            onClick={handlePrev}
                            className="absolute left-4 md:left-10 z-[60] p-3 bg-white border-4 border-black shadow-[4px_4px_0px_0px_black] hover:bg-gray-100 transition-colors group cursor-pointer"
                ...
                {/* Deck/Inpection Area */}
                <div className={`relative w-full flex items-center transition-all duration-500 ${inspectedIndex !== null ? 'justify-start max-w-5xl' : 'justify-center max-w-4xl'}`}>

                    {/* The Cards */}
                    <div className={`relative ${inspectedIndex !== null ? 'w-1/2' : 'w-full'} h-[450px] md:h-[550px] flex items-center justify-center`}>
                        {cardOrder.map((originalIndex, displayIndex) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    layout
                                    animate={{
                                        x: isCardFlying ? 1500 : (isBeingInspected ? -50 : (isVisible ? img.x : 0)),
                                    ...
                                    onDrag={(_, info) => {
                                        // If the user moves more than 5 pixels, consider it a drag, not a click
                                    ...
                                    className={`absolute w-[260px] md:w-[340px] aspect-[2.5/3.5] bg-white p-3 border-4 border-black shadow-[0px_0px_0px_0px_black] rounded-none touch-none ${isTop && inspectedIndex === null ? 'cursor-pointer' : 'pointer-events-auto'}`}
                                    style={{
                    ...
                    <AnimatePresence>
                        {inspectedIndex !== null && currentProject && (
                            <motion.div
                                ...
                                animate={{ opacity: 1, x: 0 }}
                                exit={{ opacity: 0, x: 50 }}
                                className="w-1/2 pl-12 flex flex-col justify-center"
                            >
                ...
                <AnimatePresence>
                    {inspectedIndex === null && (
                        <motion.button
                            ...
                            exit={{ opacity: 0, x: 20 }}
                            onClick={handleNext}
                            className="absolute right-4 md:right-10 z-[60] p-3 bg-white border-4 border-black shadow-[4px_4px_0px_0px_black] hover:bg-gray-100 transition-colors group cursor-pointer"
